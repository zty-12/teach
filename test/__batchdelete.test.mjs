...
// src/lib/batchDelete.ts
async function softDeleteThenPurge(table, ids) {
...
async function hardDeleteStudents(studentIds) {
  ...
  const ownedCourseIds = allCourses.filter((c) => c.studentId && ids.includes(c.studentId)).map((c) => c.id);
  await hardDeleteCourses(ownedCourseIds);
  const allMembers = await db.groupMembers.toArray();
  const memberIds = allMembers.filter((m) => ids.includes(m.studentId)).map((m) => m.id);
  await softDeleteThenPurge(db.groupMembers, memberIds);
  const allPayments = await db.payments.toArray();
  const paymentIds = allPayments.filter((p) => p.studentId && ids.includes(p.studentId)).map((p) => p.id);
  await softDeleteThenPurge(db.payments, paymentIds);
  const allStudentTags = await db.studentTags.toArray();
  const tagRelIds = allStudentTags.filter((st) => ids.includes(st.studentId)).map((st) => st.id);
  await softDeleteThenPurge(db.studentTags, tagRelIds);
  const allReports = await db.learningReports.toArray();
  const reportIds = allReports.filter((r) => ids.includes(r.studentId)).map((r) => r.id);
  await softDeleteThenPurge(db.learningReports, reportIds);
  await softDeleteThenPurge(db.students, ids);
}
async function hardDeleteCourses(courseIds) {
...
// test/__batchdelete.test.entry.ts
var pass = 0;
var fail = 0;
...
async function main() {
  console.log("=== \u573A\u666F A\uFF1A\u5220\u9664\u5B66\u751F \u21D2 \u7EA7\u8054\u6E05\u6389\u5176 1\u5BF91 \u8BFE\u7A0B/\u51FA\u5E2D/\u7ED3\u7B97 ===");
  {
    ...
    await db.payments.put(withSyncFields({
      studentId: s.id,
      amountCents: 3e4,
      method: "\u73B0\u91D1",
      paidAt: now,
    ...
    await db.studentTags.put(withSyncFields({
      studentId: s.id,
      tagId: "tag-1",
      assignedAt: now,
      createdAt: now
    ...
    ok(await db.students.get(s.id) === void 0, "\u5B66\u751F\u672C\u4F53\u5DF2\u88AB\u7269\u7406\u5220\u9664");
    ok(await db.courses.get(c.id) === void 0, "\u8BE5\u751F 1\u5BF91 \u8BFE\u7A0B\u88AB\u7EA7\u8054\u5220\u9664");
    ok(await db.courseAttendances.get(att.id) === void 0, "\u8BE5\u751F\u8BFE\u7A0B\u51FA\u5E2D\u88AB\u7EA7\u8054\u5220\u9664");
  ...
  console.log("=== \u573A\u666F B\uFF1A\u5220\u9664\u8BFE\u7A0B \u21D2 \u7EA7\u8054\u6E05\u6389\u51FA\u5E2D/\u7ED3\u7B97 ===");
  {
    ...
    const gm = [s1, s2].map((st) => withSyncFields({
      groupId: g.id,
      studentId: st.id,
      joinedAt: now,
      createdAt: now
    ...
    await db.courses.put(c);
    const atts = [s1, s2].map((st) => withSyncFields({
      courseId: c.id,
      studentId: st.id,
      present: true,
      attendAt: now,
    ...
    ok(await db.courses.get(c.id) === void 0, "\u8BFE\u7A0B\u672C\u4F53\u88AB\u7269\u7406\u5220\u9664");
    const attLeft = await db.courseAttendances.toArray();
    ok(attLeft.length === 0, "\u8BFE\u7A0B\u51FA\u5E2D\u88AB\u6E05\u9664");
    ok(await db.groups.get(g.id) !== void 0, "\u73ED\u8BFE\u672C\u8EAB\u4FDD\u7559");
    ok((await db.groupMembers.toArray()).length === 2, "\u73ED\u8BFE\u6210\u5458\u5173\u7CFB\u4FDD\u7559");
    ok(await db.students.get(s1.id) !== void 0, "\u5B66\u751F\u4FDD\u7559");
  }
  console.log(`
