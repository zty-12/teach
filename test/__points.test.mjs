...
async function doneRecordsOf(studentId) {
...
async function consecutiveDays(studentId) {
...
async function onTimeRate(studentId) {
  const dones = await doneRecordsOf(studentId);
  if (dones.length === 0) return 0;
  let onTime = 0;
  let counted = 0;
  for (const { record, task } of dones) {
    const deadline = record.dayAt ? startOfDay(new Date(record.dayAt)).getTime() + 864e5 - 1 : task.dueAt;
...
function isAllDone(records) {
  if (records.length === 0) return false;
  return records.every((r) => r.status === "done");
}
async function evaluateCondition(cond, studentId, records) {
  switch (cond.metric) {
    case "checkin_count": {
      const dones = await doneRecordsOf(studentId);
...
async function recomputeTaskPoints(taskId) {
  ...
  const rules = allRules.filter((r) => !r.deletedAt && r.enabled).sort((a, b) => a.order - b.order);
      ...
      if (rule.kind === "base") {
        ...
        rows.push(
          withSyncFields({
            ...
            delta: rule.points,
            kind: "earn",
            reason: `${task.title} \xB7 ${rule.name}`,
            taskId: task.id,
            createdAt: now
          })
        );
...
async function createCheckInTask(input) {
  const now = Date.now();
  const daySet = input.days.length > 0 ? Array.from(new Set(input.days.map((d) => startOfDay(new Date(d)).getTime()))) : input.dueAt ? [startOfDay(new Date(input.dueAt)).getTime()] : [];
  const task = withSyncFields({
  ...
  const ids = input.scope === "all" ? (input.activeStudents ?? []).map((s) => s.id) : input.memberIds;
...
async function updateCheckInRecord(record, patch) {
  const statusNext = patch.status && ["pending", "done", "missed"].includes(patch.status) ? patch.status : void 0;
  const next = {
    ...record,
    ...statusNext ? { status: statusNext } : {},
    ...patch.note !== void 0 ? { note: patch.note } : {},
    checkedAt: statusNext === "done" ? record.checkedAt ?? Date.now() : statusNext ? null : record.checkedAt
  };
  await db.checkInRecords.put(touch(next));
  await recomputeTaskPoints(record.taskId);
}

// test/__points.test.entry.ts
var pass = 0;
var fail = 0;
...
async function main() {
  ...
  console.log("\n[\u5468\u671F\u6253\u5361\u4EFB\u52A1\xB7\u591A\u5929]");
  const day1 = singleDay - 864e5 * 2;
  const day2 = singleDay - 864e5;
  const t2 = await createCheckInTask({
    ...
    scope: "all",
    note: "",
    cadenceLabel: "\u6BCF\u5929\u6253\u5361 \xB7 2 \u5929",
    memberIds: [],
    days: [day1, day2],
    activeStudents: [s1, s2]
  });
  ok(t2.created === 4, `2 \u5929 \xD7 2 \u4EBA = 4 \u6761\u8BB0\u5F55\uFF0C\u5B9E\u9645 ${t2.created}`);
