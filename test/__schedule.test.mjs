...
// test/__schedule.test.entry.shim.ts
...
// src/pages/schedule-helpers.ts
function computePlannedGroupSlots(dayStarts, groups) {
  const slots2 = [];
  for (const g of groups) {
    ...
    for (const dayStart of dayStarts) {
      ...
      const startAt = dayStart + g.startTimeMin * 6e4;
      const durationMin = g.endTimeMin > g.startTimeMin ? g.endTimeMin - g.startTimeMin : g.defaultDurationMin;
      if (durationMin <= 0) continue;
      slots2.push({
        key: `${g.id}::${startAt}`,
        groupId: g.id,
        startAt,
        endAt: startAt + durationMin * 6e4,
        durationMin
      });
    }
  }
  return slots2.sort((a, b) => a.startAt - b.startAt);
}
async function materializeGroupSlot(slot, group2, liveMembers) {
  ...
  const course = withSyncFields({
    ...
    note: "",
    status: "pending",
    colorSlot: group2.colorSlot,
    feeCents: 0,
    // 完成时按出席人数计算
    createdAt: Date.now()
  });
  await db.courses.put(course);
  const memberIds = liveMembers.filter((m) => !m.deletedAt && m.groupId === group2.id).map((m) => m.studentId);
...
function hhmmToMin(s) {
  ...
  if (h === void 0 || m === void 0 || Number.isNaN(h) || Number.isNaN(m)) return -1;
  return h * 60 + m;
}
...
function groupPlannedDuration(g) {
  if (!g) return 60;
  if (typeof g.startTimeMin === "number" && g.startTimeMin >= 0 && typeof g.endTimeMin === "number" && g.endTimeMin > g.startTimeMin) {
...
var weekStarts = Array.from({ length: 7 }, (_, i) => makeWeekStart(2026, 9, 7) + i * 864e5);
var slots = computePlannedGroupSlots(weekStarts, [group]);
var slot13to16 = slots.find((s) => s.groupId === "g1");
assert(slot13to16 !== void 0, "13:00-16:00 \u73ED\u8BFE\u751F\u6210\u8BA1\u5212\u5757");
assert(slot13to16.durationMin === 180, `\u8BA1\u5212\u5757 durationMin = 180\uFF08\u5B9E\u9645 ${slot13to16.durationMin}\uFF09`);
var expectedEnd = slot13to16.startAt + 180 * 6e4;
assert(slot13to16.endAt === expectedEnd, `endAt = startAt + 180 min\uFF08endAt - startAt = ${slot13to16.endAt - slot13to16.startAt}ms = ${(slot13to16.endAt - slot13to16.startAt) / 6e4}min\uFF09`);
async function runDbTests() {
  ...
  const groupUpdated = { ...group, endTimeMin: 900 };
  ...
  const groupNow = { ...group, endTimeMin: 960 };
  ...
  const HOUR_H = 56;
  const startHour = 8;
  function blockTopHeight(startAt, durationMin) {
